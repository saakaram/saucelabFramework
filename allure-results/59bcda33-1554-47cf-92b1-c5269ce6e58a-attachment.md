...
- Location: tests\saucedemo\cartpage.spec.ts:28:9
...
```ts
  ...
  87  |         
  88  |         if(await this.shoppoingCartIcon.isVisible()){
> 89  |             const countText = await this.cartItemCount.textContent();
      |                                                        ^ Error: locator.textContent: Test timeout of 30000ms exceeded.
  90  |             return countText ? parseInt(countText.trim()) : 0;
  91  |         }
  92  |         return 0;
  93  |     }   
  94  | 
  95  |      //logout from application
  96  |     async logout() {
  97  |         await this.hamburgerMenu.click();
  98  |         await this.logoutButton.click();
  99  |     }
  100 | 
  101 |     async getPoductCount(): Promise<number> {   
  102 |         return await this.productList.count();
  103 |     }
  104 | 
  105 |     async sortProductsBy(option: string) {
  106 |        
  107 |         await this.productSortDropdown.selectOption({ label: option });
  108 |     }
  109 | 
  110 |     async getProductPrices(): Promise<number[]> {
  111 |         const priceElements = await this.productPrice.elementHandles();
  112 |         const prices = [];
  113 |         for (const element of priceElements) {
  114 |             const priceText = await element.textContent();
  115 |             if (priceText) {
  116 |                 const price = parseFloat(priceText.replace('$', '').trim());
  117 |                 prices.push(price);
  118 |             }
  119 |         }
  120 |         return prices;
  121 |     }   
